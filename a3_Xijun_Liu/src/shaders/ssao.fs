...
void main()
{
    ...
    */
    vec3 position = texture(texturePosition, texCoords).xyz;
    ...
    * 1) Build the shading normal's frame (TBN).
         ( use getTangent() )
    */
    vec3 shading_Tangent = getTangent(normal).xyz;
    vec3 Binormal = cross(shading_Tangent, normal).xyz;
    ...
    */
    ...
    for (int i=0; i<N_SAMPLES; i++){
        vec3 sample = squareToUniformHemisphere().xyz;
        vec3 sample_World = Binormal * sample.x + shading_Tangent * sample.y + normal * sample.z;

        vec4 y = vec4(position + RADIUS * sample_World, 1.f);
        vec4 y_Screen = projection * y;
        y_Screen = y_Screen / y_Screen.w;
        vec2 y_tex = vec2(y_Screen.x/2.f + 0.5f, y_Screen.y/2.f + 0.5f);
        float depth = texture(texturePosition, y_tex).z;


        if (depth < y.z){
            color = color + vec3( INV_PI * sample.z / squareToUniformHemispherePdf());
        }
        else{
