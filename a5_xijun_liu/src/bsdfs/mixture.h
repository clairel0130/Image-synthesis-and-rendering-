...
struct MixtureBSDF : BSDF {
    ...
    v3f eval(const SurfaceInteraction& i) const override {
        ...
        if ((Frame::cosTheta(i.wi)>= 0.f)&& (Frame::cosTheta(i.wo)>= 0.f)){
            ...
            cosA = glm::dot(i.wo, MixtureBSDF::reflect(i.wi)) / ((glm::length(i.wo)) * (glm::length(MixtureBSDF::reflect(i.wi))));
            if (cosA < 0){
                cosA = 0;
            }
            cos = pow(cosA, exp);
    ...
    float pdf(const SurfaceInteraction& i) const override {
        ...
        pdf_Phong = Warp::squareToPhongLobePdf(dir, exp);
        pdf_Diffuse = Warp::squareToCosineHemispherePdf(i.wi);
        pdf = specularSamplingWeight * pdf_Phong + (1.f - specularSamplingWeight) * pdf_Diffuse;
        return pdf;
    }
    ...
    v3f sample(SurfaceInteraction& i, const v2f& _sample, float* pdf) const override {
        ...
        if(_sample.x <= specularSamplingWeight){
            sample.x = _sample.x / specularSamplingWeight;
            float exp = exponent->eval(worldData,i);
            v3f wr = i.frameNs.toWorld(normalize(reflect(i.wo)));
            v3f dir = Warp::squareToPhongLobe(sample, exp); //dir is in the lobe's local frame
            Frame lobe(wr);
            v3f dir_shading_World = lobe.toWorld(dir); // sample direction toWorld
            i.wi = i.frameNs.toLocal(normalize(dir_shading_World));
            val = MixtureBSDF::eval(i);
            *pdf = MixtureBSDF::pdf(i);
        }
        else{
            sample.x = (_sample.x - specularSamplingWeight) / (1.f - specularSamplingWeight);
