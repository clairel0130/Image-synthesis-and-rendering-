    ...
    struct GIPass : RenderPass {
        ...
        // TODO: Implement this
            ...
            for (size_t i = 0; i < obj.nVerts; i++) {
                ...
                Sampler sampler = Sampler(260654285);

                SurfaceInteraction inter;
                Ray ray(v3f(0.f), v3f(0.f, 1.f, 0.f));

                inter.wo = v3f(1.f,0.f,0.f);
                inter.p = position + normal*Epsilon;
                ...
                for (int j = 0; j < m_samplePerVertex; j++) {
                    color += m_ptIntegrator->renderExplicit(ray, sampler, inter, 0);
                }
                color /= m_samplePerVertex;

                obj.vertices[k + 3] = color.x;
                obj.vertices[k + 4] = color.y;
                obj.vertices[k + 5] = color.z;
