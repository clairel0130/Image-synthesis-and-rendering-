    ...
    if (realTime) {
        ...
        for (size_t y = 0; y < scene.config.height; y++) {
            for (size_t x = 0; x < scene.config.width; x++) {
                // for Question 1, if bonus, comment the following two lines
                Sampler sample = TinyRender::Sampler(0);
                px = (x - scene.config.width / 2.f + 0.5f) / (scene.config.width / 2.f) * aspectRatio * scaling;
                py = -((y - scene.config.height / 2.f + 0.5f) / (scene.config.height / 2.f) * scaling);
                v4f AugmentM = v4f(px, py, -1.f, 0.f);
                v4f dir = AugmentM * CameraMatrix;
                Ray ray = TinyRender::Ray(scene.config.camera.o, dir);
                pixelColor = integrator->render(ray, sample);
